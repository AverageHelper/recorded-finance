...
export async function setSession(c: Context<Env>, value: string | null): Promise<void> {
	...
	if (value === null) {
		...
		// Set session cookies
		await setSignedCookie(c, SESSION_COOKIE_NAME, value, opts);
	}
}
...
/**
 * Verifies the given JWT and extracts its payload. Throws if the JWT is
 * invalid, has expired, or if the payload is not the expected type.
 */
