...
export function jwtFromRequest(req: APIRequest, res: APIResponse): string | null {
	...
	console.debug(`Getting cookies from headers: ${JSON.stringify(req.headers)}`);
	console.debug(`and raw headers: ${JSON.stringify(req.rawHeaders)}`);
	const cookies = new Cookies(req, res, { keys, secure: true });
	const token = cookies.get(SESSION_COOKIE_NAME, { signed: true }) ?? "";
	console.debug(`Value for cookie '${SESSION_COOKIE_NAME}': ${JSON.stringify(token)}`);
	if (token) {
		return token;
	}
